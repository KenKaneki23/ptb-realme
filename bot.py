...
def when_update(update, context):
    ...
    context.job_queue.run_once(alarm(msg_id=update.message.message_id, context=chat_id),
                               10,
                               context=chat_id)
...
def alarm(context, msg_id):
    ...
    context.bot.delete_message(context=job.context, message_id=msg_id)
